...
ALTER TABLE vote_cooldowns ENABLE ROW LEVEL SECURITY;
ALTER TABLE selected_papers ENABLE ROW LEVEL SECURITY;
ALTER TABLE ip_blocks ENABLE ROW LEVEL SECURITY;
ALTER TABLE submission_order_cache ENABLE ROW LEVEL SECURITY;

-- RLS Policies for public read access
CREATE POLICY "Public can read approved submissions" ON submissions
    FOR SELECT USING (status = 'approved' OR status = 'selected');

CREATE POLICY "Public can read selected papers" ON selected_papers
    FOR SELECT USING (true);

CREATE POLICY "Public can read questions for approved submissions" ON questions
    FOR SELECT USING (
        EXISTS (
            SELECT 1 FROM submissions
            WHERE submissions.id = questions.submission_id
            AND (submissions.status = 'approved' OR submissions.status = 'selected')
        )
    );
...
CREATE POLICY "Service role can do everything on vote_cooldowns" ON vote_cooldowns
    FOR ALL USING (auth.jwt() ->> 'role' = 'service_role');

CREATE POLICY "Service role can do everything on selected_papers" ON selected_papers
    FOR ALL USING (auth.jwt() ->> 'role' = 'service_role');

CREATE POLICY "Service role can do everything on ip_blocks" ON ip_blocks
    FOR ALL USING (auth.jwt() ->> 'role' = 'service_role');

CREATE POLICY "Service role can do everything on submission_order_cache" ON submission_order_cache
    FOR ALL USING (auth.jwt() ->> 'role' = 'service_role');
